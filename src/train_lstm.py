...
def main():
    ...
    if args.resume:
        ...
        # Allow overriding training hyperparameters
        ...
        if args.num_epochs != parser.get_default('num_epochs'):
            ...
            print(f"  Overriding num_epochs: {config.num_epochs}")

        if args.batch_size != parser.get_default('batch_size'):
            config.batch_size = args.batch_size
            print(f"  Overriding batch_size: {config.batch_size}")
    ...
    if args.resume:
        ...
        if actual_input_size != config.input_size:
            ...
            print(f"  Checkpoint expects: {config.input_size} features")
            print(f"  Current preprocessor produces: {actual_input_size} features")
            print("\nYou need to adjust the preprocessor configuration to match the checkpoint.")
        ...
        if original_vocab_path.exists():
            ...
            for annotation in dataset.annotations:
                for gloss in annotation.split():
                    if gloss not in dataset.gloss_to_idx:
                        oov_glosses.add(gloss)

            if oov_glosses:
                print(f"\nWARNING: Found {len(oov_glosses)} out-of-vocabulary glosses in new data:")
                for gloss in list(oov_glosses)[:5]:
                    print(f"  - {gloss}")
                if len(oov_glosses) > 5:
                    print(f"  ... and {len(oov_glosses) - 5} more")
                print("\nThese glosses will be treated as [UNK] during training.")
        else:
