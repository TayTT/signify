...
def plot_gap_heatmap(
    ...
    sort_by:       str           = 'name',
    sort_desc:     bool          = False,
    n_bins:        int           = 100,  # normalized position buckets
):
    """
    Heatmap of gap density at normalized frame positions across all videos.
    Normalizing removes the effect of variable video lengths so you can see
    whether gaps cluster at the start/middle/end of videos structurally.
    """
    if hands not in ('both', 'left', 'right'):
        raise ValueError(f'hands must be "both", "left", or "right", got {hands!r}')

    records = _load_records(data_dir, n_videos, selection, range_start, range_end, sort_by, sort_desc)
    n_vids  = len(records)
    ...
    # build n_vids x n_bins matrix of missing fraction per bin per video
    def _build_matrix(detected_key):
        mat = np.full((n_vids, n_bins), np.nan)
        for i, r in enumerate(records):
            ...
            for b in range(n_bins):
                f_start = int(b * n / n_bins)
                f_end   = int((b + 1) * n / n_bins)
                if f_end <= f_start:
                    f_end = f_start + 1
                segment   = det[f_start:f_end]
                mat[i, b] = 1.0 - (sum(segment) / len(segment))  # missing fraction
        return mat

    fig_w = max(12.0, n_bins / 8 * figsize_scale)
    fig_h = max(4.0, (n_vids / 10 + 2) * n_subplots * figsize_scale)
    fig, axes = plt.subplots(n_subplots, 1, figsize=(fig_w, fig_h), sharex=True)
    if n_subplots == 1:
        axes = [axes]
    ...
    xticks  = np.arange(0, n_bins, xtick_step)
    xlabels = [f'{int(x / n_bins * 100)}%' for x in xticks]

    ax_idx = 0
    for detected_key, color, label in [
        ('left_detected',  COLOR_LEFT_DETECTED,  'left hand'),
        ('right_detected', COLOR_RIGHT_DETECTED, 'right hand'),
    ]:
        if (detected_key == 'left_detected'  and not show_left):
            continue
        if (detected_key == 'right_detected' and not show_right):
            continue

        mat = _build_matrix(detected_key)
        ax  = axes[ax_idx]

        im = ax.imshow(mat, aspect='auto', interpolation='nearest',
                       cmap='Reds', vmin=0.0, vmax=1.0,
                       extent=[-0.5, n_bins - 0.5, n_vids - 0.5, -0.5])
        ax.set_title(label, fontsize=10, loc='left', pad=3)
        ax.set_ylabel('video', fontsize=9)
        ax.set_yticks(range(n_vids))
        ax.set_yticklabels(names, fontsize=7)
        ax.set_xticks(xticks)
        ax.set_xticklabels(xlabels)
        plt.colorbar(im, ax=ax, label='missing fraction', shrink=0.8)
        ax_idx += 1

    axes[-1].set_xlabel('relative position in video', fontsize=10)

    # aggregate missing rate per bin across all vids for title
    def _avg_missing_by_bin(detected_key):
        vals = []
        for r in records:
            det = r[detected_key]
            n   = len(det)
            ...
            miss = [1 - sum(det[b*bucket_size:(b+1)*bucket_size]) / bucket_size
                    for b in range(n_bins)]
            vals.append(miss)
        return np.nanmean(vals, axis=0) if vals else np.zeros(n_bins)

    worst_bin_l = int(np.argmax(_avg_missing_by_bin('left_detected'))  / n_bins * 100) if show_left  else None
    worst_bin_r = int(np.argmax(_avg_missing_by_bin('right_detected')) / n_bins * 100) if show_right else None

    lines = [f'videos: {n_vids}  |  {n_bins} normalized position bins  |  color = missing fraction']
    if show_left  and worst_bin_l is not None:
        lines.append(f'left   highest avg missing around {worst_bin_l}% into the video')
    if show_right and worst_bin_r is not None:
        lines.append(f'right  highest avg missing around {worst_bin_r}% into the video')
...
COLOR_BOTH    = '#2d6a4f'  # dark green - both detected
COLOR_LEFT_ONLY  = '#457b9d'  # mid blue - left only
COLOR_RIGHT_ONLY = '#c1666b'  # mid red - right only
COLOR_NEITHER = '#aaaaaa'  # gray - neither

def plot_hand_correlation(
    data_dir:      str,
    n_videos:      int           = None,
    selection:     str           = 'random',
    range_start:   int           = 0,
    ...
    """
    Per-frame stacked bar chart of 4 detection states across all analyzed videos:
    both detected / left only / right only / neither.
    Reveals whether missing frames tend to affect both hands simultaneously
    (signer out of frame) or just one (occlusion/mediapipe failure).
    """
    records    = _load_records(data_dir, n_videos, selection, range_start, range_end, sort_by, sort_desc)
    n_vids     = len(records)
    max_frames = max(len(r['left_detected']) for r in records)

    both_count    = np.zeros(max_frames, dtype=np.int64)
    left_only     = np.zeros(max_frames, dtype=np.int64)
    right_only    = np.zeros(max_frames, dtype=np.int64)
    neither_count = np.zeros(max_frames, dtype=np.int64)
    coverage      = np.zeros(max_frames, dtype=np.int64)
    ...
    for r in records:
        ...
        n = len(ld)
        coverage[:n] += 1
        for t, (l, rr) in enumerate(zip(ld, rd)):
            if l and rr:
                both_count[t]    += 1
            elif l and not rr:
                left_only[t]     += 1
    ...
    ax.bar(frames, both_count,    width=1, align='edge', color=COLOR_BOTH,       label='both detected',  zorder=2)
    ax.bar(frames, left_only,     width=1, align='edge', color=COLOR_LEFT_ONLY,  label='left only',      zorder=2, bottom=both_count)
    ax.bar(frames, right_only,    width=1, align='edge', color=COLOR_RIGHT_ONLY, label='right only',     zorder=2, bottom=both_count + left_only)
    ax.bar(frames, neither_count, width=1, align='edge', color=COLOR_NEITHER,    label='neither',        zorder=2, bottom=both_count + left_only + right_only)

    ax.set_xlim(0, max_frames)
    ax.set_xlabel('frame (timestep)', fontsize=10)
    ax.set_ylabel('video count', fontsize=9)
    ax.xaxis.set_major_locator(ticker.MultipleLocator(max(1, max_frames // 20)))
    ax.grid(axis='y', linestyle='--', linewidth=0.4, alpha=0.4, zorder=0)
    ax.legend(loc='lower right', fontsize=9, framealpha=0.9)

    total_frames = int(coverage.sum())
    n_both    = int(both_count.sum())
    n_l_only  = int(left_only.sum())
    n_r_only  = int(right_only.sum())
    n_neither = int(neither_count.sum())

    def _pct(x):
        return x / total_frames * 100 if total_frames > 0 else 0.0

    # co-occurrence: fraction of missing frames where BOTH hands are missing
    total_missing = n_l_only + n_r_only + n_neither
    co_occurrence = n_neither / total_missing * 100 if total_missing > 0 else 0.0

    lines = [
        f'videos: {n_vids}  |  total frame slots: {total_frames}',
        f'both: {_pct(n_both):.1f}%   left only: {_pct(n_l_only):.1f}%   '
        f'right only: {_pct(n_r_only):.1f}%   neither: {_pct(n_neither):.1f}%',
        f'co-occurrence (both missing simultaneously): {co_occurrence:.1f}% of all missing frames',
    ]
    fig.suptitle('\n'.join(lines), fontsize=9, family='monospace', y=0.99, va='top',
                 ha='center', linespacing=1.6)
...
def plot_gap_scatter(
    ...
    """
    Scatter plot per video: x = number of interior gaps, y = mean interior gap length.
    Point size scales with total missing frames.
    Videos with many short gaps behave differently from those with few long ones -
    both cause similar WER but for different reasons.
    """
    if hands not in ('both', 'left', 'right'):
        raise ValueError(f'hands must be "both", "left", or "right", got {hands!r}')
    ...
    def _per_video_stats(detected_key):
        rows = []
        for r in records:
            det    = r[detected_key]
            ld, rd = r['left_detected'], r['right_detected']
            s, e   = _get_interior_range(ld, rd)
            gaps   = _find_gaps(det, s, e)
            n_gaps = len(gaps)
            avg_gap   = float(np.mean(gaps)) if gaps else 0.0
            total_mis = sum(gaps)
            rows.append((r['name'], n_gaps, avg_gap, total_mis))
        return rows

    fig_w = max(8.0, 8.0 * figsize_scale)
    fig_h = max(4.0, 4.0 * n_subplots * figsize_scale)
    fig, axes = plt.subplots(n_subplots, 1, figsize=(fig_w, fig_h), sharex=False)
    if n_subplots == 1:
        axes = [axes]
    ...
    for detected_key, color, label in [
        ...
        if detected_key == 'left_detected'  and not show_left:  continue
        if detected_key == 'right_detected' and not show_right: continue

        rows = _per_video_stats(detected_key)
        ax   = axes[ax_idx]
        names_s   = [r[0] for r in rows]
        n_gaps_s  = [r[1] for r in rows]
        avg_gap_s = [r[2] for r in rows]
        total_s   = [r[3] for r in rows]

        # size scaled to total missing frames, min size 20
        max_t  = max(total_s) if max(total_s) > 0 else 1
        sizes  = [max(20, t / max_t * 300) for t in total_s]

        sc = ax.scatter(n_gaps_s, avg_gap_s, s=sizes, c=color, alpha=0.65, zorder=3,
                        edgecolors='white', linewidths=0.5)

        ax.set_xlabel('number of interior gaps', fontsize=9)
        ax.set_ylabel('mean gap length (frames)', fontsize=9)
        ax.set_title(label, fontsize=10, loc='left', pad=3)
        ax.grid(linestyle='--', linewidth=0.4, alpha=0.4, zorder=0)

        # annotate outliers: top 5 by total missing frames
        threshold = sorted(total_s, reverse=True)[min(4, len(total_s) - 1)]
        for name, nx, ay, tot in zip(names_s, n_gaps_s, avg_gap_s, total_s):
            if tot >= threshold and tot > 0:
                ax.annotate(name, (nx, ay), fontsize=6, alpha=0.7,
                            xytext=(4, 4), textcoords='offset points')

        # size legend
        for ref_t in [int(max_t * 0.25), int(max_t * 0.75)]:
            if ref_t > 0:
                ax.scatter([], [], s=max(20, ref_t / max_t * 300), c=color, alpha=0.65,
                           label=f'{ref_t} missing frames')
        ax.legend(title='point size', fontsize=7, title_fontsize=7, loc='upper right')
        ax_idx += 1

    lines = [f'videos: {n_vids}  |  interior gaps only  |  point size = total missing frames']
    fig.suptitle('\n'.join(lines), fontsize=9, family='monospace', y=0.99, va='top',
                 ha='center', linespacing=1.6)
...
# --- 4. length vs missing rate scatter ---

def plot_length_vs_missing(
    data_dir:      str,
    n_videos:      int           = None,
    selection:     str           = 'random',
    range_start:   int           = 0,
    ...
    """
    Scatter plot per video: x = total frame count, y = interior missing rate %.
    Reveals whether longer or shorter videos have systematically worse detection.
    """
    if hands not in ('both', 'left', 'right'):
        raise ValueError(f'hands must be "both", "left", or "right", got {hands!r}')
    ...
    for detected_key, color, label in [
        ...
        for r in records:
            ...
            s, e   = _get_interior_range(ld, rd)
            intra  = det[s:e + 1]
            rate   = (1.0 - sum(intra) / len(intra)) * 100 if intra else 0.0
            lengths.append(len(det))
            miss_rate.append(rate)

        ax.scatter(lengths, miss_rate, c=color, alpha=0.6, s=30, label=label,
                   edgecolors='white', linewidths=0.4, zorder=3)

        # simple linear trend line
        if len(lengths) > 1:
            z   = np.polyfit(lengths, miss_rate, 1)
            p   = np.poly1d(z)
            xs  = np.linspace(min(lengths), max(lengths), 200)
            ax.plot(xs, p(xs), color=color, linewidth=1.0, linestyle='--', alpha=0.6)

        # pearson r in legend
        if len(lengths) > 1:
            r_val = float(np.corrcoef(lengths, miss_rate)[0, 1])
            # update last scatter label to include correlation
            handles, labels_leg = ax.get_legend_handles_labels()
            if handles:
                labels_leg[-1] = f'{label}  (r={r_val:.2f})'

    ax.set_xlabel('video length (frames)', fontsize=10)
    ax.set_ylabel('interior missing rate %', fontsize=10)
    ax.grid(linestyle='--', linewidth=0.4, alpha=0.4, zorder=0)
    ax.legend(fontsize=9, framealpha=0.9)

    fig.suptitle(
        f'videos: {n_vids}  |  interior missing rate vs video length  |  dashed = linear trend',
        fontsize=9, family='monospace', y=0.99, va='top', ha='center'
    )
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    # -- plot-gap-heatmap --
    pgh = sub.add_parser('plot-gap-heatmap',
        help='heatmap of gap density at normalized frame positions across all videos')
    _add_stat_args(pgh)
    pgh.add_argument('--hands', choices=['both', 'left', 'right'], default='both')
    pgh.add_argument('--bins', type=int, default=100, dest='n_bins',
                     help='number of normalized position buckets (default: 100)')

    # -- plot-hand-correlation --
    phc = sub.add_parser('plot-hand-correlation',
        help='per-frame stacked bar of both/left-only/right-only/neither detection states')
    _add_stat_args(phc)

    # -- plot-gap-scatter --
    pgs = sub.add_parser('plot-gap-scatter',
        help='scatter: number of gaps vs mean gap length per video, size = total missing frames')
    _add_stat_args(pgs)
    pgs.add_argument('--hands', choices=['both', 'left', 'right'], default='both')

    # -- plot-length-vs-missing --
    plm = sub.add_parser('plot-length-vs-missing',
        help='scatter: video length vs interior missing rate per video')
    _add_stat_args(plm)
    plm.add_argument('--hands', choices=['both', 'left', 'right'], default='both')
...
def _run_plot_gap_heatmap(args):
    plot_gap_heatmap(**_common_args_dict(args), hands=args.hands, n_bins=args.n_bins)

def _run_plot_hand_correlation(args):
    plot_hand_correlation(**_common_args_dict(args))

def _run_plot_gap_scatter(args):
    plot_gap_scatter(**_common_args_dict(args), hands=args.hands)

def _run_plot_length_vs_missing(args):
    plot_length_vs_missing(**_common_args_dict(args), hands=args.hands)
...
_COMMAND_HANDLERS = {
    ...
    'plot-gap-heatmap':              _run_plot_gap_heatmap,
    'plot-hand-correlation':         _run_plot_hand_correlation,
    'plot-gap-scatter':              _run_plot_gap_scatter,
    'plot-length-vs-missing':        _run_plot_length_vs_missing,
}
